...
We also showed how simple it is to create elegant and professional visuals with just one line of code.  Indeed, seaborn not only takes care of the layout, but also internally performs the semantic mapping and statistical aggregation required to create informative graphics.  Your only concern is to understand the various elements of your plots, rather than wasting  time on the technicalities of generating them.  
