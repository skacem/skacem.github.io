...
For the first example we are using `penguins` dataset.  
```python
...
As we can see from the result there is a negative linear relationship between both variables: bill length and bill depth.   
The drawback of `regplot()` is that we cannot add a hue to the plot. For that, we can use the corresponding figure-level function `lmplot()`, which combines `regplot()` and "FacetGrid".  
`FacetGrid` and figure-level plotting functions will be covered in more details in the next part of this tutorial. That said, let's take a look at what the linear regression results look like when a hue for each species of penguin is added. 
...
This is interesting. While I just claimed that there is a negative linear relationship between length and depth of penguin beaks, we now suddenly see that a strong positive linear relationship actually applies. What did we or seaborn do wrong? Well nothing really. This is a well-known phenomenon in probablity and statistics and is called the [simpson's paradox](https://en.wikipedia.org/wiki/Simpson%27s_paradox).

Another cool feature about `regplot()` is we can change the order of the regression to fit simple kinds of nonlinear relationships.  
To illustrate this we will load in the `mpg` dataset. It is about the fuel consumption in miles per gallon (mpg) for different kind of cars.
