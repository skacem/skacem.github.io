...
In the previous sections, we covered some of the common visualization techniques using mainly axes-level functions such as bar charts, histograms, scatter plots, etc.  
We also showed how simple it is to create elegant and professional visuals with just one line of code.  In fact, seaborn not only takes care of the layout, but also internally performs the semantic mapping and statistical aggregation required to create informative graphics.  Your only concern is to understand the various elements of your plots, rather than wasting  time on the technicalities of generating them.  

In this section, we are going to tackle some of the advanced concepts of seaborn such as: `Facetgrid`,  `PairGrid` and `JoinGrid` as well as the appropriate plotting techniques when dealing with complex and multi-dimensional data structures.  
...
You can easily display the five-number summary of the single hues instead of the total amount:
```python
# Show each distribution with both violins and points
sns.violinplot(x="day", y="total_bill", 
               hue="smoker", data=tips, 
...
Cool right?  
I guess by now you too agree with xkcd comic about violins.
